package com.cfw.m1212.web.home.configuration;
...
public class RemoteServiceInjectConfiguration {
    ...
    @Bean
    public AmqpProxyExtendedFactoryBean injectRemoteTypeService() throws Exception {
        AmqpProxyExtendedFactoryBean factoryBean = new AmqpProxyExtendedFactoryBean();
        factoryBean.setServiceInterface(TypeService.class);
        factoryBean.setRoutingKey("m1212-server-type-rpc-queue");
        factoryBean.setAmqpTemplate(rabbitTemplate);
        factoryBean.setMessagePostProcesser(messageExtendedPostProcesser);
